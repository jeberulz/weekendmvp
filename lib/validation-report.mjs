...
export function completeDateRanges(now = new Date(), dataLagDays = 2) {
  if (!Number.isInteger(dataLagDays) || dataLagDays < 1) {
    throw new Error("dataLagDays must be a positive integer");
  }
  const today = new Date(
    Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()),
  );
  const currentEnd = shiftUtcDate(today, -dataLagDays);
...
export function renderValidationMarkdown(report) {
  const lines = [
    ...
    `Data scope: consented client-side GA4 events; the most recent ${report.ranges.dataLagDays} days are excluded for processing latency.`,
    "",
    "## Signals",
    "",
  ];
  ...
  lines.push(
    ...
    "Purchase completion is a site-level return-page signal; Stripe and Convex remain authoritative for paid orders.",
    "",
...
<html><body style="font-family:Arial,sans-serif;color:#171717;line-height:1.5;max-width:760px;margin:0 auto;padding:24px">
  ...
  <strong>Baseline:</strong> ${html(report.ranges.baseline.startDate)} → ${html(report.ranges.baseline.endDate)}<br>
  <strong>Scope:</strong> Consented client-side GA4 events; latest ${report.ranges.dataLagDays} days excluded for processing latency.</p>
  <h2>Signals</h2>
  ${signalRows}
  <h2>Funnel</h2>
  <table style="border-collapse:collapse;width:100%">
