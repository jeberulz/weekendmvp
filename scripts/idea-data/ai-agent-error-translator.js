// Phase 2 fresh idea — sourced from Ideabrowser #7458 (Doctor Debug).
module.exports = {
  ...
  tagline: 'The error message, rewritten for the person who actually has to fix it.',
  shortDescription:
    'Connect your AI agent once. When it breaks, Doctor Debug explains why in plain English and tells you exactly how to fix it, with zero documentation reading.',
  solutionShort:
    'A monitoring layer for no-code AI agents that turns cryptic engineer error logs into plain-English explanations and step-by-step fixes.',
  ...
  audiences: ['developers', 'solo-founders', 'non-technical'],
  ...
  steps: [
    'Integrate one platform first (Lindy) via its webhook and observability API, and ingest a real failure event into a normalized error shape.',
    'Build the translation engine: map the error signature to a plain-English explanation plus a ranked fix path, using Claude over a growing pattern library.',
    'Ship a dashboard and Slack alert that fire the explanation the moment an agent breaks, with a one-tap was-this-fix-right feedback loop.',
  ],
  ...
  competitors: [
    { name: 'Native platform logs', pricing: 'Included', note: 'Lindy and n8n expose raw errors, but they are written for engineers and offer no plain-English fix path.' },
    { name: 'Sentry', pricing: 'from $26/mo', note: 'Excellent error monitoring, but aimed at developers debugging their own code, not operators running no-code agents.' },
    { name: 'Zapier error emails', pricing: 'Included', note: 'Notifies you something failed, with little context and no guided resolution for the specific agent.' },
    { name: 'Generic ChatGPT', pricing: '$20/mo', note: 'Can sometimes explain an error if you paste it, but has no live connection to the agent and no fix library.' },
  ],
  competitiveGap:
  ...
  pricing: [
    { name: 'Solo', price: '$15/mo', desc: 'One connected agent, real-time plain-English error explanations. The validation tier.' },
    { name: 'Agency', price: '$49-99/mo', desc: 'Multiple agents, shared dashboard, Slack alerts, and audit logs for client work.' },
    { name: 'Platform', price: 'Custom', desc: 'Embedded observability licensed to the workflow platforms themselves to cut churn.' },
  ],
  unitEconomics: [
    { value: '$49/mo', label: 'Agency tier price' },
    { value: '~100', label: 'Agencies to $5K MRR' },
    { value: '< $1', label: 'AI cost per resolved error' },
  ...
  tech: [
    { layer: 'Ingestion', choice: 'Platform webhooks + observability APIs', why: 'Captures failure events from Lindy and n8n without asking the user to touch code.' },
    { layer: 'AI', choice: 'Claude + a growing pattern library', why: 'Maps an error signature to a plain-English cause and a ranked fix path, improving as resolutions accrue.' },
    { layer: 'Backend', choice: 'Cloudflare Workers + Supabase', why: 'Cheap, fast event handling and storage for runs, patterns, and feedback.' },
    { layer: 'Delivery', choice: 'Web dashboard + Slack alerts', why: 'Meets operators where they already watch for problems and notifies the moment an agent breaks.' },
  ],
  ...
  prompts: {
    ...
    extraTitle: 'Pattern Library Feedback',
    extra:
  ...
  related: [
    { slug: 'ai-feedback-triage-widget', title: 'AI Feedback Triage Widget', category: 'developer-tools' },
