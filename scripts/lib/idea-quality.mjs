...
/** Hard floor — below this, the page is still thin even if filler-free. */
export const MIN_DEEP_BODY_WORDS_HARD = 1800;
...
export function findHygieneIssues(prose) {
  ...
  if (/How it works:[^\n]*[a-z]/i.test(prose) && !/\*\*How it works:\*\*/.test(prose)) {
    // bare "How it works:" run-on without the bold label is ok if bold exists;
    // flag obvious run-ons like "How it works: Foo Bar Baz Step"
  }
  // Mid-sentence Capital Audience chunks: "for Smb SaaS Sales" style after "for "
  // Kept light — only flag repeated Title Case audience dump.
  if (/\bfor [A-Z][a-z]+ [A-Z][a-z]+ [A-Z][a-z]+ [A-Z][a-z]+\b/.test(prose)) {
    issues.push(
      "possible mid-sentence Title Case audience dump (lowercase audience names in prose)",
    );
  }
  return issues;
}
