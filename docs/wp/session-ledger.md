...
| 2026-08-05 | `/root` | Orchestrator | High | Freeze WP21 auth compatibility, security, provider, routing, and production boundaries | `codex/wp21-convex-auth`, shared checkout | WP21 stories/progress, registry/session orchestration docs | In progress | Not recorded | Official current Convex Auth docs reviewed; Resend decision remains owner-only; no production action. |
| 2026-08-05 | `/root/wp21_worker` | Worker | High | WP21-S1 through S5: isolated Convex Auth compatibility migration and auth/route tests | `codex/wp21-convex-auth`, shared checkout | Frozen worker boundary in `wp21-stories.md` | Planned | Not recorded | Must stop on provider ruling, production target, destructive schema change, cookie broadening, or tenant-routing expansion. |
