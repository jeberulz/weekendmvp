...
## 2026-08-06 - WP25-S6 authenticated browser gate: pass

- Same live session as WP23-S6: the owner's real Resend magic-link sign-in in a gstack `browse`-connected visible local Chrome window against `http://localhost:3000` and the local Convex backend. No production/cloud endpoint touched.
- Own-idea intake at `/dashboard/new`: filled all five fields; `localStorage` held only `weekendmvp:own-idea-intake` (a random UUID) — never brief text, identity, or authoritative state, matching the frozen contract. "Draft saved" appeared after the queued autosave.
- Refresh/resume: reloaded the tab; all five field values came back exactly as typed, resumed from the server draft via `getOwnIdeaDraftByKey`, not from local storage content.
- Review → Confirm: "Review brief" moved DOM focus to the `<h1>Confirm the brief</h1>` heading; the review screen echoed every field verbatim; keyboard `Tab` order from that heading was "Edit answers" then "Confirm this revision" (logical, linear). Confirming produced a real server-side navigation to `/dashboard/projects/{projectId}`, landing on a genuine "Brief confirmed / Revision 1 confirmed" project page with an "Edit as a new revision" action.
- Two-tab conflicting first-save: cleared the intake key, opened two fresh tabs at `/dashboard/new` with no existing draft, typed different working titles in each ~0.3s apart (each tab's own 800ms autosave debounce independent). Both converged on the same server-issued idempotency key (by design — `intakeKey()` reads-then-writes `localStorage` synchronously, so the second tab's later debounce picks up the first tab's already-written key rather than minting its own), then raced two `startOwnIdea`-family calls with **different input** under that one key. Result: exactly one project record was created (verified via `/dashboard/projects` — no duplicate), both tabs reactively converged to display the same canonical text, and neither tab was left showing "Draft saved" next to text that didn't match the server's canonical value. This confirms the no-duplicate, no-false-save-with-mismatched-text guarantee live; note this reproduces the "shared-key, differing-input, sequential-resolution" case reliably — the sub-millisecond simultaneous-creation interleaving is covered only by the existing Convex `Promise.all` test (`convex/wp25Intake.test.ts`), not by this live pass, and that distinction is recorded rather than glossed over.
- Responsive layout: `/dashboard/new` at 390x844 renders as a single stacked column with the fixed bottom nav docked below the form (confirmed via a viewport-only screenshot, not the full-page one, which visually duplicates fixed elements at their scroll offset — a screenshot artifact, not a layout bug).
- Project cards: `/dashboard/projects` showed exactly the two real records created in this session ("Wave 2 QA Gate Test Idea" — Brief confirmed, and the race-test draft) with real status labels and update timestamps — no invented completion percentage, artifact, or activity.
- Automated accessibility: `axe-core@4.12.1` injected live, wcag2a/wcag2aa/wcag21a/wcag21aa ruleset. Zero violations on `/dashboard/new` (mobile, 25 passes) and `/dashboard/projects/{projectId}` (desktop, 24 passes).
- Standard gate re-run after the browser pass (no code changed): `npm run typecheck` pass; `npm run lint` pass, 0 errors, same 35 pre-existing warnings; `npm test` pass (full suite incl. 364 Convex tests); `npm run build` pass, 310 pages, only pre-existing warnings; `npm audit --omit=dev --audit-level=high` 0 vulnerabilities; `git diff --check` clean.
- Two local-dev-only test projects remain in the local Convex deployment as a byproduct of this evidence pass (no archive/delete UI exists yet — consistent with the WP22 no-destructive-deletion-in-v1 ruling). This is local dev data only; no production record was touched.
- **WP25-S6 is complete.**
