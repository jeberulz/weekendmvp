...
## WP26-S1 Contract Subgate - 2026-08-06

- **Work package:** WP26 durable task workflow and M3 Validation Reports — `S1` only (versioned Validation Report and site-input contracts). `S2`-`S6` are not in scope of this gate.
- **Branch:** `codex/wp26-research-workflow`
- **Environment:** Local repo only; no Convex deployment beyond the local `convex dev` process auto-regenerating `convex/_generated/api.d.ts` to register the new module. No cloud/production deployment, provider credential, or environment mutation.
- **Key finding that shaped the implementation:** WP22 already froze `tasks`, `task_steps`, `documents`, `document_citations`, and `workflow_runs` tables, with `"validation_report"` already present in `TASK_TYPE_VALUES`/`WORKFLOW_TYPE_VALUES`/`DOCUMENT_KIND_VALUES` and `MAX_GENERATED_DOCUMENT_BODY_BYTES`/`assertGeneratedDocumentBody` already enforcing the 256 KiB generated-document cap. `S1` therefore required **zero `convex/schema.ts` changes** — only new application-level contract code at `convex/platform/engine/contracts.ts`, following the existing `briefPayload.ts` JSON-in-`documents.body` convention. The independent reviewer separately re-confirmed this premise against the frozen schema/validators rather than taking the claim on trust.
- **What shipped:** `ValidationReportPayload` (v1 engine scope per program-platform-plan.md §4.5) and `SiteInputPayload` (the decoupled shape WP27 will consume), both versioned via an embedded `contractVersion`, both with fail-closed `parse`/`serialize` pairs, plus `assertValidationReportCitationsInRange`/`assertSiteInputCitationsInRange` cross-checks against the real `document_citations` row count. Citations are stored as `position` references into `document_citations`, never duplicated into the JSON body.
- **Independent review:** A separate fresh subagent (not the implementing session) reviewed for correctness, findings-first, per this program's orchestration rules. Found one Medium bug (keyword `volume`/`cpc` accepted `Infinity` via a raw `1e400` JSON literal — not reachable through the normal serialize path, but reachable if a later story parses raw provider output directly) and one minor asymmetry (no citation-range check existed for `SiteInputPayload`). Both fixed; a design note about the `document_citations.position` bigint vs. `CitationRef` number type was recorded in `docs/wp/wp26-progress.md` for `S5` (the first story that will actually write/compare these) rather than fixed now, since `S1` never touches `ctx.db`. Re-review of the fixes confirmed no remaining issue and no regression.
- **Checks run:** `npx vitest run convex/platform/engine/contracts.test.ts` — 24 passed; `npm run typecheck` pass; `npx eslint convex/platform/engine/` pass, 0 findings; full `npm test` pass (45 test files, 490 tests, 0 failures); `npm run build` pass, 310 pages; `npm audit --omit=dev --audit-level=high` 0 vulnerabilities; `git diff --check` clean; scoped secret-pattern scan on the new files, no matches.
- **Result:** **Pass.** `WP26-S1` is complete and independently reviewed.
- **Next-wave authorization:** Per the manifest, WP27 may now begin in parallel with WP26's remaining `S2`-`S6` work, once WP27's own dependency (WP25, already passed) and any WP27-specific preconditions are otherwise satisfied. This does not authorize `S2`-`S6` implementation to skip their own stories, tests, or review; it only unblocks WP27 specifically. **Update, same day:** the two remaining WP26 provider ruling gaps (exact OpenAI model ID, keyword-data provider) were closed by the owner — `gpt-5.6-sol` and DataForSEO, see `docs/wp/RULINGS.md`. `S2` through `S6` are now unblocked as well.

## WP27 Package Gate - 2026-08-06

- **Work package:** WP27 structured site renderer and isolated preview (`S1`–`S6`)
- **Branch:** `codex/wp27-site-preview`
- **Environment:** Local Next.js (`:3000` for auth journey matching Convex `SITE_URL`; production `next build` + `next start` on `:3100` for header/cache evidence) and local Convex (`:3210`) only. No cloud/production deploy, host cutover, publish, live lead capture, payment, or credential rotation.
- **Owner rulings closed during gate:** claim-only read exclusivity after claim (2026-08-06); decorative preview watermark WCAG 1.4.3 exemption (earlier same day). Both in `docs/wp/RULINGS.md`.
- **Standard checks (recorded at S6 pre-closeout):** `npm run typecheck` pass; `npm run lint` 0 errors / 35 pre-existing warnings; `npm test` pass — node groups 91/6/7/46/4, vitest groups 125/172/57/567; `npm run build` pass, 312 pages; `npm audit --omit=dev --audit-level=high` 0 vulnerabilities; `git diff --check` clean; secret scan clean across the branch diff. `convex/schema.ts` remains **31 insertions / 0 deletions** (one additive `preview_capabilities` table).
- **Anonymous journey + abuse/XSS/a11y:** previously recorded live against production build — generate all three templates, preview render, rate-limit/content-type/origin gates, six XSS payloads clean, axe per-template desktop+mobile with only the owner-exempted watermark and pre-existing ConsentBanner contrast, keyboard-only pass. Soft-404 (HTTP 200 with not-found body) under Cache Components PPR documented and accepted as non-oracle.
- **Authenticated journey (closeout evidence):** preview → Keep this site → `/signin?claimPreview=…` (sessionStorage stash confirmed) → owner magic-link → `/dashboard`. Server: `platform/preview/read:view` → `claimed: true`; `npx convex data projects` → exactly **1** `wp27:preview:` idempotency key (`Verify any collectible in under a minute`, `repository_idea`, `draft`). Dashboard reload still shows that single claimed project plus pre-existing own-idea rows — no duplicate claim.
- **Independent review:** prior high-risk review findings either fixed in-package or explicitly recorded (retention cron MEDIUM — required before public free-preview exposure, new scope; bridge nonce/expiry LOW; ConsentBanner contrast out of WP27). No unresolved critical/high/medium in capability security, XSS/injection, caching/indexing, ownership, or claim idempotency.
- **Result:** **Pass.** WP27-S1 through WP27-S6 complete. Package gate closed.
- **Next-wave authorization:** WP28 (host routing, publish, live leads) may open per manifest sequencing. WP26-S2..S6 remain independently unblocked on `codex/wp26-research-workflow`. Do not merge WP27 to `main` without the platform program merge plan — `/build/{slug}` is required for already-merged CTAs. Pending follow-up before public free-preview: `preview_capabilities` retention job. WP38 admin plan must still merge before production activation.

## WP28 Package Gate - 2026-08-07

- **Work package:** WP28 tenant publish, host routing, versions, and leads (`S1`–`S6`)
- **Branch:** `codex/wp28-tenant-hosts`, branched from `codex/wp27-site-preview` @ `0a13b2b`. Not merged to `main`; the platform program remains unmerged.
- **Environment:** Local Next.js production builds (`next build` + `next start -p 3100`/`:3105`) and a local `local:` Convex deployment only. No cloud or production deploy, no DNS or domain change, no wildcard, no certificate, no live customer site, no payment, no external send, no credential rotation.
- **What shipped:** a pure host classifier with a frozen 40-name reserved list (`lib/tenant-host.ts`); host classification in `middleware.ts` ahead of every other hop, with unknown and reserved hosts answering a genuine 404 issued from middleware rather than `notFound()`; the tenant route `app/site/[slug]` reusing the WP27 renderer with `showPreviewChrome={false}`; `platform/sites/read` (public host→site resolution), `platform/sites/publish` (publish/rollback/unpublish), and `platform/sites/leads` (synthetic-only capture); `lib/tenant-publish-check.ts`; and `docs/wp/wp28-activation-runbook.md`.
- **Schema:** `git diff 0a13b2b..HEAD -- convex/schema.ts` is **empty**. WP28 is a zero-diff package for the serialized one-writer schema seam, as planned at the story freeze and confirmed independently.
- **Owner rulings applied (7, all 2026-08-07, `docs/wp/RULINGS.md`):** legacy fallback removed (bare 404); the 40-name reserved list frozen; tenant leads synthetic-only; Vercel preview deployments as the staging host; fail-open retained with a bare tenant not-found boundary; and the `S4` concurrency/idempotency ACs re-scoped to what a `convex-test` harness can prove.
- **Independent review:** commissioned 2026-08-07 as a separate agent with no authorship of the code. Returned **0 CRITICAL, 3 HIGH, 3 MEDIUM, 8 LOW**. It falsified three claims recorded in `docs/wp/wp28-progress.md` — the fail-open failure mode, the tenant route's "no platform navigation" contract, and a concurrency test comment claiming contention the harness cannot create. All three HIGH and two of three MEDIUM were remediated and re-verified live (`6e588e8`); the third MEDIUM was closed by the AC re-scoping ruling. No unresolved critical, high, or medium finding remains.
- **Reviewer-verified clean, by execution:** host confusion and header spoofing across 15 combinations (`X-Forwarded-Host` in both directions, `Forwarded`, duplicate `Host`, embedded credentials, trailing dot, empty, uppercase, non-standard port, lookalike parent) — Next 16's `getHostname` reads `headers.host` only; tenant→platform isolation with identical 9-byte 404s and no `location` or `set-cookie`; cookie scope including forged auth cookies presented to a tenant host; self-canonical tenant head with zero `www.weekendmvp.app`; `showPreviewChrome` required (mutation-tested twice); lead ownership including forged owner/project bodies; and all five new suites confirmed running in the **full** `npm test`. Test-quality pass found no vacuous `api`-proxy assertions and no escaped-`onerror` false positives.
- **Standard checks (post-remediation):** `npm run typecheck` exit 0; `npm run lint` exit 0, 0 errors and 35 pre-existing warnings (unchanged, none in WP28 files); `npm test` exit 0 — node groups 91/6/29/80/4, vitest groups 159/172/84/624; `npm run build` exit 0, 315 pages; `npm audit --omit=dev --audit-level=high` 0 vulnerabilities; `git diff --check` clean.
- **Result:** **Pass.** WP28-S1 through WP28-S6 complete. Package gate closed.
- **Accepted residuals, recorded not hidden:** during a Convex outage a tenant host answers 200 with a bare, unbranded, `noindex` 458-byte document rather than a 404 (owner ruling; failing closed would dark every customer site on one blip); the RSC flight payload still carries ~25 KB of root not-found markup, which is neither visible nor rendered and would need multiple root layouts to remove; `/_next/static/*` answers 200 on any host because the middleware matcher excludes it (reviewer independently concurred, LOW); the publish check adds one uncached backend round trip per tenant request; and lead rate limiting is keyed per IP and therefore shared across tenants (harmless while leads are synthetic).
- **Dry run:** the activation runbook's host matrix, tenant page assertions, lead endpoint matrix, publish/rollback/unpublish, and pre-flight check were all executed against a local production build with real Convex data. **Deploying the branch to a Vercel preview was not executed** — it requires pushing a currently local-only branch, which is an owner decision — and is recorded in the runbook as an explicit gap, because Vercel's edge sets both `Host` and `X-Forwarded-Host` while middleware reads `Host`.
- **Next-wave authorization:** WP29 (project cockpit) may open per manifest sequencing; WP26-S2..S6 remain independently unblocked on `codex/wp26-research-workflow`. This does **not** authorize production activation. WP31 alone owns wildcard DNS, domains, certificates, and live customer sites, and must not activate a wildcard against any deployment that fails the runbook's §5 pre-flight check. WP38 must still merge before production activation. Still required before the anonymous free preview is public: the WP27 `preview_capabilities` retention job (no `convex/crons.ts` exists yet). Real tenant lead capture remains a WP31 item gated on privacy and retention text.
- **Evidence:** `docs/wp/wp28-stories.md`, `docs/wp/wp28-progress.md`, `docs/wp/wp28-activation-runbook.md`, `docs/wp/RULINGS.md` (7 WP28 rows).
