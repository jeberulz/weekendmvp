...
- **Passed foundation checks:** typecheck; lint with 0 errors/35 inherited warnings; auth 26/26; Convex 41/41; redirects 32/32; full configured suite; 303-page build; full and production npm audits with 0 vulnerabilities; diff check; staged secret scan with 0 hits.
...
- **Completion blockers:** Configure `AUTH_RESEND_KEY`, a verified `AUTH_RESEND_FROM`, and the required safe log level in the isolated deployment, then pass one real inbox issue/confirmation/session/logout flow; configure Google OAuth credentials securely and pass real redirect/callback/session/logout. Production remains separately blocked on fresh inventory, full backup, restore tag, dry run, secrets, and owner approval.
