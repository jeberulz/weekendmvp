...
- **Next-wave authorization:** Per the manifest, WP27 may now begin in parallel with WP26's remaining `S2`-`S6` work, once WP27's own dependency (WP25, already passed) and any WP27-specific preconditions are otherwise satisfied. This does not authorize `S2`-`S6` implementation to skip their own stories, tests, or review; it only unblocks WP27 specifically. `S2` (provider adapters) remains blocked pending two open ruling gaps — the exact OpenAI model ID and a named keyword-data provider — per `docs/wp/wp26-stories.md`'s "Ruled Inputs" section.
