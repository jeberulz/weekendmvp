...
| WP19 | Playbooks lead-magnet microsites | Work Package | `claude/lead-magnet-ideas-mvp-hhsajy` | Complete; deploy pending | Config-driven `/playbooks/{slug}` engine renders chrome-free framework microsites; The Decision Stack ships as the first instance with a free server-rendered body plus an email-gated pack; `playbook` allowlisted in `ALLOWED_UTM_CAMPAIGNS`; sitemap, canonical, JSON-LD and the `/decision-stack` short alias wired; configured checks green. |
