...
| WP21 | Convex Auth compatibility foundation green; provider E2E blocked | 2 | `codex/wp21-convex-auth` | Awaiting owner input; production blocked | Compatibility schema, live-session denial, Google/static contracts, canonical routing, build, audits, and independent review pass. Completion still requires an owner-selected magic-link vendor/sender plus configured Google and email-provider E2E. |
