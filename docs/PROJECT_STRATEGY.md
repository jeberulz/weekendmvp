...
| WP11 | Convex database I/O audit and safe fixes | Work Package | `cursor/deploy-wp11-convex-perf-d2d5` | Shipping to prod | Hot Convex reads are traced end-to-end, safe indexed/bounded fixes preserve related-idea eligibility and ordering, local-only parity tests and project gates pass, and the audit documents expected monthly I/O below 250 MB without a migration. Merge + Vercel prod deploy + `npx convex deploy` required for the backend I/O win. |
