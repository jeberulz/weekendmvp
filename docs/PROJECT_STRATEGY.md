...
| WP22 | Platform schema, state, and authorization | 2 | `codex/wp22-platform-contracts` | Complete — contract gate passed | Additive indexed contracts and two-user ownership/state-machine tests are frozen for parallel consumers. |
| WP23 | Signed-in shell, dashboard, Explore, and intent | 2 | `codex/wp23-platform-explore` | Active — parallel worktree | Approved responsive/a11y shell and full Explore views persist owner-scoped state over canonical idea records. |
| WP24 | Credits and Stripe | 2 | `codex/wp24-credits-stripe` | Active — parallel worktree | Purpose-separated test Checkout and exact-once account/ledger/refund invariants pass adversarial tests. |
| WP25 | Intake, briefs, projects, and public idea bridge | 2 | `codex/wp25-intake-projects` | Active — parallel worktree | Repository and own-idea paths create a confirmed owned brief/project without SEO regression. |
