# Article Topics Research Bank

Last updated: 2026-01-22

---

## Keyword Research Summary

### Market Context (2026)

- Global AI market projected to reach $347.05 billion in 2026 (31.5% CAGR)
- SaaS industry hitting $300 billion by 2026
- Micro-SaaS segment growing 30% annually ($15.7B → $59.6B by 2030)
- 1 in 4 Americans have a side hustle
- 70% of top-ranking content displays strong E-E-A-T signals
- AI-generated content performing worse - readers and search engines detecting it

### Hot Search Terms

- "micro saas ideas 2026"
- "build app no code weekend"
- "AI tools build MVP non-technical"
- "side hustle ideas 9 to 5"
- "vibe coding cursor claude bolt"
- "validate startup idea quickly"
- "when to quit job start startup"

### Key Pain Points (from research)

1. Decision paralysis - too many ideas, can't pick
2. Scope creep - ideas too big to finish
3. Validation anxiety - fear of building wrong thing
4. Technical barrier - think they need to code
5. Time constraint - only have weekends
6. Co-founder myth - think they need a technical partner

---

## Topics Queue

### STATUS KEY
- [ ] = Not started
- [~] = In progress
- [x] = Published

---

### Tier 1: High Volume Keywords

#### Topic 1
- [x] **PUBLISHED**
- **Title:** Stop Overthinking, Start Building
- **Slug:** stop-overthinking-start-building
- **Framework:** PAS
- **Published:** 2026-01-22
- **File:** articles/stop-overthinking-start-building.html

#### Topic 2
- [x] **PUBLISHED**
- **Title:** How to Build Your First App in a Weekend (Even If You Can't Code)
- **Slug:** how-to-build-your-first-app-in-a-weekend
- **Framework:** How-To Tutorial
- **Published:** 2026-01-22
- **File:** articles/how-to-build-your-first-app-in-a-weekend.html

#### Topic 3
- [x] **PUBLISHED**
- **Title:** 7 Micro-SaaS Ideas You Can Build Solo in 2026
- **Slug:** 7-micro-saas-ideas-solo-2026
- **Framework:** Listicle
- **Published:** 2026-01-22
- **File:** articles/7-micro-saas-ideas-solo-2026.html
- **Keywords:** "micro saas ideas 2026", "solo founder saas"
- **Research Sources:**
  - https://lovable.dev/guides/micro-saas-ideas-for-solopreneurs-2026
  - https://www.knack.com/blog/no-code-micro-saas-ideas/
  - https://elementor.com/blog/profitable-saas-micro-saas-ideas/

#### Topic 4
- [x] **PUBLISHED**
- **Title:** How to Validate a Startup Idea in 48 Hours (Before Writing Any Code)
- **Slug:** validate-startup-idea-48-hours
- **Framework:** How-To Tutorial
- **Published:** 2026-01-22
- **File:** articles/validate-startup-idea-48-hours.html
- **Keywords:** "validate startup idea quickly", "startup validation no code"
- **Research Sources:**
  - https://review.firstround.com/unconventional-tactics-for-validating-your-startup-idea/
  - https://webflow.com/blog/validate-startup-ideas
  - https://martian.ventures/mantra/insights/innovative-idea-validation-methods-in-product-design-from-fake-doors-to/

#### Topic 5
- [x] **PUBLISHED**
- **Title:** The $1K/Month Idea: Why You Don't Need a Billion-Dollar Startup
- **Slug:** the-1k-month-idea
- **Framework:** Myth-Busting
- **Published:** 2026-01-23
- **File:** articles/the-1k-month-idea.html
- **Keywords:** "small startup ideas", "micro saas revenue", "$1000 month business"
- **Research Sources:**
  - https://elementor.com/blog/profitable-saas-micro-saas-ideas/
  - https://firstsiteguide.com/micro-saas-ideas/
  - https://www.demandsage.com/startup-statistics/
  - https://abhyashsuchi.in/startup-playbook-2026-bootstrap/

---

### Tier 2: Medium Volume, Lower Competition

#### Topic 6
- [ ] **QUEUED**
- **Title:** Vibe Coding 101: Build Real Apps with Claude, Cursor, and Bolt
- **Keywords:** "vibe coding", "cursor claude bolt tutorial", "AI coding tools 2026"
- **Framework:** How-To Tutorial
- **Search Volume:** Medium (trending up)
- **Notes:** "Vibe coding" is the hot new term. Cover tool comparison: Cursor (4.9/5 rating), Claude Code (handles 50k+ LOC), Bolt.new (browser-based). Include: getting started steps, when to use which tool.
- **Research Sources:**
  - https://www.humai.blog/i-built-a-full-app-with-cursor-in-2-hours-heres-my-step-by-step-guide-to-vibe-coding-in-2026/
  - https://www.nucamp.co/blog/top-10-vibe-coding-tools-in-2026-cursor-copilot-claude-code-more
  - https://dev.to/jimmyshoe85/my-real-vibe-coding-workflow-claude-projects-bolt-vs-code-no-cursor-required

#### Topic 7
- [ ] **QUEUED**
- **Title:** 5 Side Projects That Actually Make Money (From People Who Built Them)
- **Keywords:** "side projects that make money", "profitable side hustle ideas"
- **Framework:** Listicle with case study elements
- **Search Volume:** Medium
- **Notes:** Include real examples. Digital products ($500-$5k/mo), faceless YouTube, UGC creation ($50-$150/video), AI automation services ($60-$150/hr), online courses.
- **Research Sources:**
  - https://www.meriwest.com/our-story/blog/8-side-hustles-build-passive-income-2026
  - https://www.entrepreneur.com/starting-a-business/8-creative-side-hustles-from-2025-to-make-you-money-in-2026/500886
  - https://jobright.ai/blog/best-side-hustles-2026/

#### Topic 8
- [ ] **QUEUED**
- **Title:** When to Quit Your Job for Your Side Project (5 Signs You're Ready)
- **Keywords:** "quit job start business", "side hustle to full time"
- **Framework:** Listicle or BAB
- **Search Volume:** Medium
- **Notes:** Signs: 12-18 months expenses saved, revenue exceeds job income, validated business model, time constraints hurting both, strong support network. Include realistic caveats.
- **Research Sources:**
  - https://www.zenbusiness.com/blog/7-signs-that-you-should-quit-your-job-and-start-a-business/
  - https://foundr.com/articles/building-a-business/when-to-quit-your-job
  - https://www.liveplan.com/blog/starting/questions-before-quitting-job-to-start-business

#### Topic 9
- [ ] **QUEUED**
- **Title:** Why ChatGPT Can't Give You Good Startup Ideas (And What Actually Works)
- **Keywords:** "chatgpt startup ideas", "AI brainstorm ideas bad"
- **Framework:** Myth-Busting
- **Search Volume:** Medium
- **Notes:** Contrarian angle. ChatGPT gives generic, unvalidated ideas. What works: problem-first thinking, niche focus, personal experience, market research. Position Weekend MVP library as the solution.
- **Angle:** Challenge the "just ask AI" crowd

#### Topic 10
- [ ] **QUEUED**
- **Title:** The 3-Screen MVP: The Only Framework You Need to Ship This Weekend
- **Keywords:** "mvp framework", "simple mvp", "weekend mvp"
- **Framework:** How-To Tutorial
- **Search Volume:** Medium
- **Notes:** Introduce Weekend MVP's core methodology. 3 screens: Landing → Input → Output. Examples of successful 3-screen apps. Why less is more. Branded content.
- **Internal Link Value:** High (explains core methodology)

---

### Tier 3: Niche / Authority Building

#### Topic 11
- [ ] **QUEUED**
- **Title:** The Real Cost of Not Shipping: A Founder's Calculation
- **Keywords:** "opportunity cost startup", "why ship fast"
- **Framework:** BAB or Hero's Journey
- **Search Volume:** Low
- **Notes:** Emotional piece. Calculate: time spent planning vs building, ideas that went stale, competitors who shipped first. End with urgency to act.

#### Topic 12
- [ ] **QUEUED**
- **Title:** Build in Public in 2026: What's Changed (And What Still Works)
- **Keywords:** "build in public 2026", "indie hacker strategy"
- **Framework:** Inverted Pyramid or Myth-Busting
- **Search Volume:** Low-Medium
- **Notes:** Build in public isn't dead, it's evolving. "Share early, hide later" strategy. When to go quiet. Avoid building for other indie hackers.
- **Research Sources:**
  - https://www.indiehackers.com/post/lifestyle/is-this-the-end-of-build-in-public-heres-why-top-indie-hackers-are-suddenly-disappearing
  - https://thesolopreneur.blog/posts/on-buildinpublic-and-indiehackers/

#### Topic 13
- [ ] **QUEUED**
- **Title:** The Non-Technical Founder's Stack in 2026
- **Keywords:** "non-technical founder tools", "no code stack 2026"
- **Framework:** Listicle
- **Search Volume:** Low-Medium
- **Notes:** Complete tool stack: Cursor/Bolt for building, Claude for thinking, Vercel for hosting, Stripe for payments, Beehiiv for email. Cost breakdown.

#### Topic 14
- [ ] **QUEUED**
- **Title:** Ship It Saturday: A Framework for Weekend Builders
- **Keywords:** "weekend project framework", "ship fast methodology"
- **Framework:** How-To Tutorial
- **Search Volume:** Low
- **Notes:** Branded framework piece. Friday night planning, Saturday building, Sunday shipping. Create a repeatable ritual. Community angle.

---

## Research Data Points (Use in Articles)

### Market Stats
- Global AI market: $347.05B by 2026 (31.5% CAGR)
- SaaS market: $300B by 2026
- Micro-SaaS segment: $15.7B → $59.6B by 2030 (30% growth)
- 42% of startups fail because they don't solve a real problem
- 1 in 4 Americans have a side hustle
- AI tools reduce MVP time from 6 months to 2-6 weeks
- One fintech founder: quoted $80K/4 months, got MVP in 8 days for <$2K

### Tool Stats
- Cursor: ~4.9/5 user rating
- Claude Code: Handles 50k+ LOC codebases 75% of the time
- Bolt.new: Browser-based, no local setup required
- Successful micro-SaaS: $5K-$30K MRR typical

### Quotes to Use
- "Make Something People Want" - YC
- "The strongest validation: when someone asks 'how much and when can I buy it?'"
- "2026 is the first year where literally anyone can build software"

---

## Publishing Schedule Recommendation

**Week 1:**
- Topic 3: 7 Micro-SaaS Ideas (Listicle - high volume)

**Week 2:**
- Topic 6: Vibe Coding 101 (Tutorial - trending term)

**Week 3:**
- Topic 4: Validate Idea in 48 Hours (Tutorial - high volume)

**Week 4:**
- Topic 9: Why ChatGPT Can't Give Good Ideas (Myth-Busting - contrarian)

---

## How to Update This File

After publishing an article:
1. Change `[ ]` to `[x]` for that topic
2. Add `**PUBLISHED**` status
3. Add publish date and file path
4. Move to "Published" section if desired
