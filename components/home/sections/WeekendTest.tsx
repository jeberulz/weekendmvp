...
export function WeekendTest({ averageHours }: { averageHours: number }) {
  return (
    <section aria-labelledby="home-test-title" data-scene="test" className="relative overflow-hidden bg-home-paper">
      ...
      <Container className="relative flex flex-col gap-8 py-14 lg:gap-14 lg:py-24">
        <div data-m="head" className="flex flex-col gap-3.5 lg:gap-4">
          ...
          <h2 id="home-test-title" data-m="lines" className="font-editorial text-[38px] font-normal leading-[1.02] tracking-[-0.025em] text-home-ink lg:text-[64px] lg:leading-[1.06] lg:tracking-[-0.02em]">
            What makes an idea <Em>weekend-sized.</Em>
          </h2>
        </div>
        <div data-m="wall" className="grid grid-cols-1 gap-8 lg:grid-cols-[minmax(0,1fr)_200px] lg:items-center">
          <ul className="grid grid-cols-2 gap-x-3.5 gap-y-[18px] lg:grid-cols-3 lg:gap-x-10 lg:gap-y-12">
            {WEEKEND_TEST.map((note, i) => (
              ...
              >
                <span aria-hidden data-m="pin" className="absolute left-1/2 top-[9px] size-[11px] -translate-x-1/2 rounded-full bg-home-orange shadow-[0_2px_3px_rgba(0,0,0,0.3)] lg:top-3 lg:size-3.5" />
          ...
          <div data-m="seal" className="flex items-center gap-[18px] lg:flex-col lg:gap-[22px] lg:text-center">
